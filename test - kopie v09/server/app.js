...
app.post('/send-offer', (req, res) => {
    ...
    const mailOptions = {
        ...
        html: `
            ...
            <p>Telefoon: ${offer.phone}</p>
            <p>Adres: ${offer.address}</p>
            <h3>Samengestelde producten:</h3>
            <ul>
                ${offer.products.map(item => `<li>${item.name} - €${item.price}</li>`).join('')}
            </ul>
            <p><strong>Totaalprijs: €${offer.total}</strong></p>
        `
    };
    ...
    mg.messages().send(mailOptions, (error, body) => {
        ...
        console.log('E-mail succesvol verzonden:', body);
        res.status(200).json({ success: true, message: 'Offerte is verstuurd' });
    });
});
